...
export default function HookComponent2() {
    ...
    const inputRef = useRef<HTMLInputElement>(null);    // inputRef => HTMLInputElemnet | undefined

    const onButtonClickHandler = () => {
        // ref의 참조 객체는 해당 변수의 current 속성에 들어있음

        // 참조변수(null 혹은 undefined를 가질수 있는 object타입의 변수)에서 특정 속성 및 메서드를 호출하려 할때
        // null 혹은 undefined가 아닌 상태에서만 작업을 수행하도록 하려면 참조변수?.속성() 혹은 참조변수?.함수()로 실행
        // inputRef.current?.focus();
        if (inputRef.current) inputRef.current.focus();
    }

    const onInputChangeHandler = () => {
        const { current } = inputRef;       // 구조화파괴
        if (!current) return;
