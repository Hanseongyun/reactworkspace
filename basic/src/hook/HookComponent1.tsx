...
// - useReducer : 상태 로직을 관리하는 함수, 상태 업데이트 로직을 컴포넌트 외부로 옮길 수 있게하여 코드관리를 수월하게 해줌
// - useCallback : 메모리 내에서 함수를 재사용할 수 있도록 하는 함수, 랜더링 없이 특정 함수를 다시 호출할 수 있도록 해줌
// - useMemo : 메모리에 계산된 값(함수의 결과)을 저장하여 함수 재실행 없이 사용할 수 있도록 하는 함수

// - 훅 함수는 반드시 컴포넌트의 코드 블럭에 존재해야 함
// - 만약 컴포넌트의 코드 블럭 외에 존재하고 싶다면 그 코드 블럭은 반드시 훅 함수 이어야 함

// const [count, setCount] = useState<number>(0);   // 밖에 선언하면 에러뜸
...
export default function HookComponent1() {
    ...
    const addCount = () => {
        // const [count, setCount] = useState<number>(0);   // 여기도 에러
        setCount(count +1);
    };
    ...
    useEffect(() => {
        // 스코프할 상태 배영을 지정하지 않은 effect에서 상태를 변경하면 무한 실행됨(주의)
    ...
    useEffect(() => {
        // 스코프할 상태 배열에 지정한 상태를 해당 effect에서 변경하면 무한 실행됨(주의)
        // setCount(count + 1);
