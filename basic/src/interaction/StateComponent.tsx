...
export default function StateComponent() {
    ...
    // let count = 0;
    const [count, setCount] = useState<number>(0);      // 반드시 const로 선언
    // let total: number = 0;
    ...
    const oncountAddHandler = () => {
        ...
        // setCount((count => count + 1));
        // setCount((count => count + 1));        // 이까지 3
        // setCount(count +1);                    // 다시 1로 바뀜

        // 변경된 상태를 사용하고 싶을 때 해결방법, 임시변수를 사용하여 간접 사용
        const tmp = count + 1;          // 작업시 한템포씩 밀리면 의심
        setCount(tmp);
        setTotal(total + tmp);
    };
    ...
    const onAddListHandler = () => {
        // 상태가 배열 혹은 객체이면 각각에 대해 요소를 추가, 변경이 일어나도 상태가 변경된 것으로 인식하지 않음 (실제 주소 변경x)
        // 상태가 변경된 것으로 인식하게 하고 싶으면 새로운 배열 혹은 객체를 생성하여 상태를 변경해야 함 (일반적으로 복사해서 변경)

        // favorites.push(favorite);           // .push : 추가
        // const newFavorites = favorites.map(item => item);   // 배열을 복사해서 새로운 배열생성
        // setFavorites(newFavorites);            // 배열은 참조변수- 주소를 보여줌, 랜더링이 되야 바뀜(배열을 새로만들면 주소가 바뀜 > 랜더링)

        setFavorites([...favorites, favorite]);     // spread 연산자 이용
        setFavorite('');                            // 추가를 누르면 빈문자열로 변경
    };
